...
const SeatLayoutManager: React.FC<SeatLayoutManagerProps> = ({ roomId, readOnly = false, onLayoutChange }) => {
  ...
  // Current total seats
  const currentTotalSeats = calculateTotalSeats(
    bulkConfig.rowsInput,
    bulkConfig.columnsPerRow,
    bulkConfig.emptyColumns
  );
  const hasValidationErrors = Object.values(validationErrors).some((error) => error !== "");
  const isFormValid =
    !hasValidationErrors &&
    bulkConfig.rowsInput.trim() &&
    bulkConfig.columnsPerRow &&
    currentTotalSeats >= 50 &&
    currentTotalSeats <= 150;
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-slate-700 text-white rounded-lg border ${
                  validationErrors.rowsInput ? "border-red-500" : "border-[#FFD875]/30"
              ...
              {validationErrors.rowsInput ? (
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Nhập theo dạng A-E (từ A đến E) hoặc A,B,C,D,E (liệt kê từng hàng)
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-slate-700 text-white rounded-lg border ${
                  validationErrors.columnsPerRow ? "border-red-500" : "border-[#FFD875]/30"
                } focus:border-[#FFD875] focus:ring focus:ring-[#FFD875]/20 focus:outline-none transition-all duration-300 shadow-[0_0_10px_0px_rgba(255,216,117,0.2)] focus:shadow-[0_0_15px_0px_rgba(255,216,117,0.4)]`}
                min="5"
                max="20"
            ...
            <div>
              <label className="block text-sm font-medium text-[#FFD875] mb-2">Loại ghế</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-[#FFD875] mb-2">Cột trống (tùy chọn)</label>
              <input
                type="text"
              ...
              />
              <p className="text-xs text-gray-400 mt-1">
                Tạo lối đi bằng cách bỏ trống các cột (ví dụ: 5,6 sẽ tạo lối đi ở cột 5 và 6)
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`font-medium px-6 py-3 rounded-lg transition-all duration-300 flex items-center gap-2 ${
                !bulkConfig.rowsInput.trim() || !bulkConfig.columnsPerRow || !isFormValid
