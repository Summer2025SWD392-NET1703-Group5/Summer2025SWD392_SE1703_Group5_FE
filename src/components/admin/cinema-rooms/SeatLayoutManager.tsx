                ...
                >
                    ...
                    <div className="bg-slate-800 rounded-lg p-4 border border-slate-700 space-y-3">
                        <div className="flex items-center gap-2 mb-3">
                            <svg className="w-5 h-5 text-[#FFD875]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                            </svg>
                            <h3 className="text-[#FFD875] font-semibold text-sm">Thống kê phòng chiếu</h3>
                        </div>

                        {/* Thông tin cơ bản */}
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-slate-700/50 rounded-lg p-3 text-center">
                                <div className="text-xs text-gray-400 mb-1">Loại phòng</div>
                                <div className="text-[#FFD875] font-bold text-lg">{roomDetails?.Room_Type || '2D'}</div>
                            </div>

                            <div className="bg-slate-700/50 rounded-lg p-3 text-center">
                                <div className="text-xs text-gray-400 mb-1">Tổng số ghế</div>
                                <div className="text-[#FFD875] font-bold text-lg">{seatMap?.stats?.total_seats || 0}</div>
                            </div>

                            {/* Trạng thái chỉnh sửa */}
                            {seatMap?.can_modify !== undefined && (
                                <div className="bg-slate-700/50 rounded-lg p-3 text-center">
                                    <div className="text-xs text-gray-400 mb-1">Trạng thái</div>
                                    <div className="flex items-center justify-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${seatMap.can_modify ? 'bg-green-500' : 'bg-red-500'}`} />
                                        <span className={`text-xs font-medium ${seatMap.can_modify ? 'text-green-400' : 'text-red-400'}`}>
                                            {seatMap.can_modify ? 'Có thể sửa' : 'Không thể sửa'}
                                        </span>
                                    </div>
                                </div>
                            )}
                        ...
                        {seatMap?.stats?.seat_types && seatMap.stats.seat_types.length > 0 && (
                            <div className="space-y-2">
                                <div className="text-xs text-gray-400 font-medium">Phân loại ghế:</div>
                                <div className="flex flex-wrap gap-3">
                                    {seatMap.stats.seat_types.map((seatType, index) => (
                                        <div key={index} className="bg-slate-700/50 rounded-lg px-3 py-2 flex items-center gap-2">
                                            ...
                                            <span className="text-gray-300 text-sm font-medium">
                                                {seatType.SeatType === 'VIP' ? 'VIP' :
                                                 seatType.SeatType === 'Regular' ? 'Thường' :
                                                 seatType.SeatType}
                                            </span>
                                            <span className={`font-bold text-sm ${
                                                seatType.SeatType === 'VIP' ? 'text-purple-400' :
                                                seatType.SeatType === 'Regular' ? 'text-green-400' :
                                                'text-gray-400'
                                            }`}>
                                                {seatType.Count}
                                            </span>
