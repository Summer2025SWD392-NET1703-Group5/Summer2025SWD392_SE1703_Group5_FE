...
const roomSchema = yup.object({
  ...
  Notes: yup.string().max(500, "Ghi chú không được vượt quá 500 ký tự").nullable().optional(),
});
...
const CinemaRoomForm: React.FC<CinemaRoomFormProps> = ({ room, onSubmit, onCancel, loading = false, cinemaId }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      Status:
        room?.Status === "Deleted" || room?.Status === "Closed" || room?.Status === "Maintenance"
          ? "Inactive"
          : room?.Status || "Active",
      Notes: room?.Notes || "",
    },
          ...
          parsedRows.forEach((expectedRow, rowIdx) => {
            ...
            for (let col = 1; col <= seatLayoutConfig.seatsPerRow; col++) {
              ...
              for (const apiRow of data.data.rows) {
                const seat = apiRow.Seats.find(
                  (s: any) => s.Row_Label.toLowerCase() === expectedRow.toLowerCase() && s.Column_Number === col
                );
                if (seat) {
                  foundSeat = seat;
                  break;
              ...
              if (foundSeat) {
                ...
                if (col <= 3) {
                  // Log 3 ghế đầu mỗi hàng để debug
                  console.log(
                    `🪑 Ghế ${foundSeat.Row_Label}${foundSeat.Column_Number}: seatIndex=${seatIndex}, Layout_ID=${foundSeat.Layout_ID}`
                  );
                }
              } else {
  ...
  const handleToggleHiddenSeat = useCallback(
    async (seatIndex: number) => {
      ...
      if (roomId) {
        try {
          ...
          if (seatLayout) {
            console.log(
              `🎯 Toggle ghế ${seatIndex}: ${isCurrentlyHidden ? "hiện" : "ẩn"} ghế ${seatLayout.Row_Label}${
                seatLayout.Column_Number
              }`
            );
            ...
            try {
              ...
              if (foundSeat) {
                console.log(
                  `🎯 Tìm thấy ghế và toggle: ${isCurrentlyHidden ? "hiện" : "ẩn"} ghế ${foundSeat.Row_Label}${
                    foundSeat.Column_Number
                  }`
                );
  ...
  const handleSeatTypeChange = useCallback(
    async (seatIndex: number, newType: "Regular" | "VIP") => {
      // Cập nhật state local trước
      setSeatLayoutConfig((prev) => {
      ...
      if (roomId) {
        try {
          ...
          if (seatLayout) {
            console.log(
              `🎯 Đổi loại ghế ${seatIndex}: ${seatLayout.Row_Label}${seatLayout.Column_Number} thành ${newType}`
            );
            ...
            toast.success(
              `Đã cập nhật ghế ${seatLayout.Row_Label}${seatLayout.Column_Number} thành ${
                newType === "VIP" ? "VIP" : "Thường"
              }`
            );
          } else {
            // Nếu không có mapping, load lại dữ liệu và tìm ghế để cập nhật
            ...
            try {
              ...
              if (foundSeat) {
                console.log(
                  `🎯 Tìm thấy ghế và đổi loại: ${foundSeat.Row_Label}${foundSeat.Column_Number} thành ${newType}`
                );
                ...
                toast.success(
                  `Đã cập nhật ghế ${foundSeat.Row_Label}${foundSeat.Column_Number} thành ${
                    newType === "VIP" ? "VIP" : "Thường"
                  }`
                );
  ...
  const handleStatsUpdate = useCallback(
    (stats: { total: number; regular: number; vip: number; hidden: number }) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
                  ...
                  return (
                    <div key={step.id} className="flex items-center">
                      ...
                      >
                        <div
                          className={`p-2 rounded-lg ${
                            isActive ? "bg-[#FFD875]/20" : isCompleted ? "bg-green-500/20" : "bg-slate-600/50"
                          }`}
                        >
                          {isCompleted ? (
                            <CheckCircleIcon className="h-5 w-5 text-green-400" />
                          ) : (
                            <StepIcon className={`h-5 w-5 ${isActive ? "text-[#FFD875]" : "text-gray-400"}`} />
                          )}
                        </div>
                        <span
                          className={`font-medium ${
                            isActive ? "text-[#FFD875]" : isCompleted ? "text-green-400" : "text-gray-400"
                          }`}
                        >
                          {step.label}
                        </span>
                      ...
                      {index < steps.length - 1 && (
                        <div className={`w-8 h-0.5 mx-2 ${isCompleted ? "bg-green-500/50" : "bg-slate-600/50"}`}></div>
                      )}
                    </div>
                  );
                })}
      ...
      {currentStep === "preview" && isNewRoom ? (
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          >
                            ...
                            <div className="text-white font-bold text-sm mt-1">
                              {parseRowsInput(seatLayoutConfig.rowsInput).length}
                            </div>
                          </motion.div>
                          <motion.div
                            className="bg-slate-700/40 rounded-lg p-2 border border-slate-600/30"
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <motion.div
            className={`${
              currentStep === "preview" && isNewRoom ? "hidden" : "xl:col-span-2"
            } bg-slate-800/80 backdrop-blur-sm rounded-2xl p-6 border border-[#FFD875]/20 shadow-2xl`}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.2 }}
          >
                      ...
                      <form onSubmit={handleSubmit(handleFormSubmit as any)} className="space-y-6">
                            ...
                            render={({ field }) => (
                              <div className="relative">
                                ...
                                <input
                                  {...field}
                                  type="text"
                                  className={`w-full pl-12 pr-4 py-4 bg-slate-700/50 backdrop-blur-sm text-white rounded-xl border ${
                                    errors.Room_Name || duplicateNameError ? "border-red-500" : "border-[#FFD875]/30"
                        ...
                        <div>
                            ...
                            render={({ field }) => (
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  type="number"
                                  className={`w-full pl-12 pr-4 py-4 bg-slate-700/50 backdrop-blur-sm text-white rounded-xl border ${
                                    errors.Seat_Quantity ? "border-red-500" : "border-[#FFD875]/30"
                                  } focus:border-[#FFD875] focus:ring-2 focus:ring-[#FFD875]/20 focus:outline-none transition-all duration-300 shadow-lg hover:shadow-xl`}
                                  ...
                                  readOnly={
                                    !!(
                                      isNewRoom &&
                                      seatLayoutConfig.rowsInput.trim() &&
                                      seatLayoutConfig.seatsPerRow > 0
                                    )
                                  }
                                />
                          ...
                          {errors.Seat_Quantity && (
                            <p className="mt-2 text-sm text-red-400">{errors.Seat_Quantity.message}</p>
                          )}
                          {isNewRoom && seatLayoutConfig.rowsInput.trim() && seatLayoutConfig.seatsPerRow > 0 && (
                      ...
                      <div className="space-y-6">
                          ...
                          {seatLayoutConfig.rowsInput && parseRowsInput(seatLayoutConfig.rowsInput).length > 0 && (
                            ...
                            >
                              <p className="text-xs text-[#FFD875] bg-[#FFD875]/10 px-3 py-2 rounded-lg">
                                ✓ Sẽ tạo {parseRowsInput(seatLayoutConfig.rowsInput).length} hàng:{" "}
                                {parseRowsInput(seatLayoutConfig.rowsInput).join(", ")}
                              </p>
                            </motion.div>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setSeatLayoutConfig((prev) => ({ ...prev, seatsPerRow: parseInt(e.target.value) || 0 }))
                            }
                            className="w-full px-4 py-4 bg-slate-700/50 backdrop-blur-sm text-white rounded-xl border border-[#FFD875]/30 focus:border-[#FFD875] focus:ring-2 focus:ring-[#FFD875]/20 focus:outline-none transition-all duration-300 shadow-lg hover:shadow-xl"
                        ...
                        {/* Seat Type */}
                        <div>
                          <label className="block text-sm font-medium text-[#FFD875] mb-3">Loại ghế mặc định</label>
                          <div className="grid grid-cols-2 gap-3">
                            <motion.button
                              type="button"
          ...
          <motion.div
            className={`${
              currentStep === "preview" && isNewRoom ? "w-full" : "xl:col-span-1"
            } bg-slate-800/80 backdrop-blur-sm rounded-2xl ${
              currentStep === "preview" && isNewRoom ? "p-8" : "p-4"
            } border border-[#FFD875]/20 shadow-2xl`}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="relative">
              ...
              <div className="relative">
                <h3
                  className={`${
                    currentStep === "preview" && isNewRoom ? "text-2xl" : "text-lg"
                  } font-bold text-white mb-3 flex items-center gap-2`}
                >
                      ...
                      <div className="bg-slate-700/30 rounded-lg p-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          ...
                          <div>
                            ...
                            <span className="text-white ml-1">
                              {seatLayoutConfig.seatType === "VIP" ? "VIP" : "Thường"}
                            </span>
                          </div>
                          <div>
                                ...
                                >
                                  ...
                                  <div className="flex gap-1">
                                    {row.Seats.map((seat) => (
                                      <motion.div
                                        key={seat.Layout_ID}
                                        className={`w-8 h-8 rounded-lg border-2 flex items-center justify-center text-xs transition-all duration-200 ${getSeatTypeColor(
                                          seat.Seat_Type
                                        )} ${!seat.Is_Active ? "opacity-50" : ""}`}
                            ...
                            <div className="bg-slate-700/30 rounded-xl p-4 mb-4">
                              ...
                              <div className="mt-3 flex justify-center gap-6 text-xs">
                                {seatLayout.stats.seat_types.map((type) => (
                                  <span
                                    key={type.SeatType}
                                    className="flex items-center gap-2 bg-slate-600/30 px-3 py-1 rounded-lg"
                                  >
                                    <span className="text-lg">{getSeatTypeIcon(type.SeatType)}</span>
                                    <span className="text-white">
                                      {type.SeatType}: <span className="font-bold">{type.Count}</span>
                                    </span>
                                  </span>
                                ))}
                              </div>
                        ...
                        <div className="text-center py-8">
                          <div className="bg-slate-700/30 rounded-xl p-6">
                            ...
                            <p className="text-sm text-gray-400">
                              Sử dụng nút bên dưới để cấu hình sơ đồ ghế cho phòng chiếu
                            </p>
                          </div>
                        </div>
                      )}
                ...
                {isNewRoom &&
                  (currentStep === "basic" || currentStep === "layout") &&
                  seatLayoutConfig.rowsInput.trim() &&
                  seatLayoutConfig.seatsPerRow > 0 && (
                    <motion.div
                      initial={{ opacity: 0, scale: 0.95 }}
