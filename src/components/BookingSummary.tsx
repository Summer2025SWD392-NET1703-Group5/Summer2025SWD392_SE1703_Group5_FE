...
const BookingSummary: React.FC<BookingSummaryProps> = ({
  ...
  return (
    <div className="bg-slate-800 rounded-xl p-6 sticky top-4">
      {/* Timer */}
      <div className={`flex items-center justify-center space-x-2 mb-6 p-3 bg-slate-700 rounded-lg ${timeLeft < 60 ? 'text-red-500' : 'text-gray-300'
        }`}>
      ...
      <div className="space-y-3">
        <button
          ...
          className={`w-full bg-yellow-600 hover:bg-yellow-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-medium py-3 px-4 rounded-lg transition-colors ${isExpired || selectedSeats.length === 0
              ? 'bg-gray-700 text-gray-400 cursor-not-allowed'
              : ''
            }`}
        >
          {isExpired ? 'Hết thời gian đặt vé' : selectedSeats.length === 0 ? 'Chọn ghế để tiếp tục' : 'Tiếp tục thanh toán'}
        </button>
      ...
      <div className="mt-6 text-xs text-gray-400 space-y-1">
        ...
        <p>• Ghế sẽ được giữ trong {Math.floor(timeLeft / 60)} phút</p>
