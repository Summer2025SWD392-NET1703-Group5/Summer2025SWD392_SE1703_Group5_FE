...
import { ChevronLeftIcon, HomeIcon, CubeIcon, SparklesIcon } from "@heroicons/react/24/outline";
  ...
  useEffect(() => {
    ...
    const loadRoomDetails = async () => {
      try {
        ...
        // Get cinema information from URL search params
        if (cinemaId) {
          try {
            const cinemaData = await cinemaService.getCinemaById(parseInt(cinemaId));
            setCinema(cinemaData);
          } catch (error) {
            console.error("Error loading cinema details:", error);
            // Don't block the page if cinema details fail to load
            setCinema(null);
          }
        }
      } catch (error: any) {
        ...
        >
          ...
          <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <p className="text-white font-medium text-lg">{cinema?.Cinema_Name || "Không xác định"}</p>
            </motion.div>
